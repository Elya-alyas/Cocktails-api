...
const renderCocktails = (drinks) => {
    ...
    drinks.map(drink => {
        ...
        name.textContent = drink.strDrink.length > 20 ? drink.strDrink.slice(0, 20) + '...' : drink.strDrink
...
const renderRandom = (drinks) => {
    ...
    drinks.map(drink => {
        ...
        textCocktail.classList.add('textCocktail', 'animate__animated', 'animate__fadeInRight')
...
const renderDetail = (drink) => {
    ...
    for (let key in drink) {
        if (key.includes('strIngredient') && drink[key] !== null) {
            ...
            ingr.textContent = drink[key]
            ul.append(ingr)
            ingr.addEventListener('click', () => getIngrByName(drink[key], drink.idDrink))
        }
    }
    const backBtn = document.createElement('button')
    backBtn.textContent = 'Back'
...
// ингридиенты, кнопка при ингридиентах, кнопка рандом и выходит рандомный коктейль, и вывести фото ингридиентов и анимацию